<?php
// admin/manage-users.php - Complete user management (Admin)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!CSRF::validateToken($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_POST['create_user'])) {
            ...
            // Admin role restrictions - cannot create admin or superadmin accounts
            if ($currentUser['role'] === 'admin' && in_array($role, ['admin', 'superadmin'])) {
                $message = 'You can only create regular user accounts.';
                $messageType = 'error';
            } else {
                // Validation
        ...
        elseif (isset($_POST['update_user'])) {
            ...
            if (!$targetUser) {
                ...
                $messageType = 'error';
            } elseif ($currentUser['role'] === 'admin' && in_array($targetUser['role'], ['admin', 'superadmin'])) {
                $message = 'Cannot modify admin or superadmin accounts.';
                $messageType = 'error';
            } elseif ($currentUser['role'] === 'admin' && in_array($role, ['admin', 'superadmin'])) {
                $message = 'You can only set role to regular user.';
                $messageType = 'error';
            } else {
                // Validation
                if (!Validator::validateUsername($username)) {
        ...
        elseif (isset($_POST['delete_user'])) {
            ...
            if (!$targetUser) {
                $message = 'User not found.';
                $messageType = 'error';
            } elseif (in_array($targetUser['role'], ['admin', 'superadmin'])) {
                $message = 'Cannot delete admin or superadmin accounts.';
                $messageType = 'error';
...
// Get users with filtering - admins only see regular users
$searchTerm = $_GET['search'] ?? '';
$roleFilter = $_GET['role'] ?? '';
$activeFilter = $_GET['active'] ?? '';

// For admins, restrict to users only
if ($currentUser['role'] === 'admin') {
    $users = $userObj->getFiltered('user', $activeFilter, $searchTerm);
} else {
        ...
        <main class="main-content">
            ...
            <div class="top-bar">
                <div class="page-title">
                    <h1>Manage Users</h1>
                    <p><?php echo $currentUser['role'] === 'admin' ? 'User management - Create, Edit, Delete regular users' : 'Complete user management - Create, Edit, Delete users'; ?> (Total: <?php echo count($users); ?>)</p>
                </div>
            ...
            <div class="dashboard-content">
                ...
                <?php if ($currentUser['role'] === 'admin'): ?>
                    <div class="admin-restriction">
                        ...
                        <span><strong>Admin Notice:</strong> You can only create and manage regular user accounts. Admin and superadmin accounts require superadmin privileges.</span>
                    </div>
                <?php endif; ?>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    ...
                                                    if ($currentUser['role'] === 'superadmin') {
                                                        $canEdit = $user['role'] !== 'superadmin' || $user['id'] == $currentUser['id'];
                                                    } else {
                                                    ...
                                                    $canDelete = false;
                                                    if ($currentUser['role'] === 'superadmin') {
                                                        $canDelete = $user['role'] !== 'superadmin' && $user['id'] != $currentUser['id'];
                                                    } else {
                                                        $canDelete = $user['role'] === 'user' && $user['id'] != $currentUser['id'];
                                                    }
                                                    ?>
                                                    <?php if ($canDelete): ?>
        ...
        if (table) {
            function filterAndSortTable() {
                ...
                rows.forEach(row => {
                    ...
                    const roleCell = <?php echo $currentUser['role'] === 'superadmin' ? '3' : '2'; ?>;
                    const role = (row.cells[roleCell] ? row.cells[roleCell].textContent : '').toLowerCase();
                    const statusCell = <?php echo $currentUser['role'] === 'superadmin' ? '4' : '3'; ?>;
                    const status = (row.cells[statusCell] ? row.cells[statusCell].textContent : '').toLowerCase();
                ...
                visibleRows.sort((a, b) => {
                    ...
                    switch(sortValue) {
                        ...
                        case 'created-desc':
                        case 'created-asc':
                            const createdCell = <?php echo $currentUser['role'] === 'superadmin' ? '5' : '4'; ?>;
                            aVal = new Date(a.cells[createdCell] ? a.cells[createdCell].textContent : '');
                            bVal = new Date(b.cells[createdCell] ? b.cells[createdCell].textContent : '');
                            break;
                        case 'login-desc':
                            const loginCell = <?php echo $currentUser['role'] === 'superadmin' ? '6' : '5'; ?>;
                            aVal = new Date(a.cells[loginCell] ? a.cells[loginCell].textContent : '');
                            bVal = new Date(b.cells[loginCell] ? b.cells[loginCell].textContent : '');
                            break;
                    }
            ...
            if (roleFilter) roleFilter.addEventListener('change', filterAndSortTable);
